        ...
        <DashboardLayout>
            ...
            <div className="fixed bottom-6 right-6 z-40 flex items-end gap-3">
                {aiTooltipVisible && !aiPanelOpen && (
                    <div className="relative bg-white rounded-xl shadow-lg border border-blue-100 p-3 pr-8 max-w-[200px]">
                        <button onClick={() => setAiTooltipVisible(false)} className="absolute top-2 right-2 p-1 hover:bg-gray-100 rounded text-gray-400"><X size={12} /></button>
                        <p className="text-xs font-medium text-gray-800">Need help?</p>
                        <p className="text-[11px] text-gray-500">Ask Fluxor AI about your store</p>
                        <div className="absolute -right-2 bottom-4 w-0 h-0 border-t-8 border-b-8 border-l-8 border-transparent border-l-white" />
                    </div>
                )}
                <div className="relative">
                    <div className="absolute inset-0 rounded-full animate-pulse" style={{ background: 'radial-gradient(circle, rgba(59,130,246,0.3) 0%, transparent 70%)', transform: 'scale(2)', filter: 'blur(8px)' }} />
                    <button
                        onClick={() => { setAiPanelOpen(true); setAiTooltipVisible(false); }}
                        className="relative w-12 h-12 bg-blue-600 hover:bg-blue-500 rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-110"
                        title="Ask Fluxor AI"
                    >
            ...
            {aiPanelOpen && (
                <div className="fixed inset-0 z-50 flex justify-end">
                    <div className="absolute inset-0 bg-black/20" onClick={() => setAiPanelOpen(false)} />
                    <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col">
                        <div className="flex items-center justify-between p-4 border-b border-blue-100">
                            ...
                            <button onClick={() => setAiPanelOpen(false)} className="p-2 hover:bg-gray-100 rounded-lg"><X size={18} className="text-gray-500" /></button>
                        </div>
                        <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                            <div className="flex gap-3">
                                <div className="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center flex-shrink-0"><Bot size={14} className="text-blue-500" /></div>
                                <div className="bg-blue-50 rounded-xl rounded-tl-sm px-4 py-3 max-w-[85%]">
                                    <p className="text-sm text-gray-700">
                                        {actionItems.length > 0 ? `${actionItems.length} items need attention. Want me to help prioritize?` : "Everything looks good. I'm monitoring your store."}
                                    </p>
                                </div>
                            </div>
                            {chatMessages.map((msg, i) => (
                                <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${msg.role === 'user' ? 'bg-blue-600' : 'bg-blue-50'}`}>
                                        {msg.role === 'user' ? <span className="text-white text-xs font-semibold">{userName?.[0]?.toUpperCase() || 'U'}</span> : <Bot size={14} className="text-blue-500" />}
                                    </div>
                                    <div className={`rounded-xl px-4 py-3 max-w-[85%] ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-blue-50 text-gray-700 rounded-tl-sm'}`}>
                                        <div className="text-sm">{msg.role === 'user' ? msg.content : formatAIMessage(msg.content)}</div>
                                    </div>
                                </div>
                            ))}
                            {chatLoading && (
                                <div className="flex gap-3">
                                    <div className="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center flex-shrink-0"><Bot size={14} className="text-blue-500" /></div>
                                    <div className="bg-blue-50 rounded-xl rounded-tl-sm px-4 py-3">
                        ...
                        <div className="p-4 border-t border-blue-100">
                            <div className="relative">
                                <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyPress={handleKeyPress}
                                    placeholder="Ask about your store..." disabled={chatLoading}
                                    className="w-full py-3 px-4 pr-12 bg-blue-50 border border-blue-100 rounded-xl text-sm focus:outline-none focus:border-blue-400 disabled:opacity-50" />
                                <button onClick={sendMessage} disabled={chatLoading || !chatInput.trim()}
                                    className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-blue-600 hover:bg-blue-500 rounded-lg flex items-center justify-center disabled:opacity-50">
